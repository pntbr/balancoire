...
export function venteAvoirEcriture(line, numeroCompte, lastEcritureNum) {
    ...
    if (['Association'].includes(line['qui paye ?'])) {
        ...
        const label = `vente : ${line['note'] || line['qui paye ?'] }`;
        const ecritures = [
...
export function venteStripeEcriture(line, numeroCompte, lastEcritureNum) {
    return [
        ...
        creationEcriture({ JournalCode: 'VT', EcritureNum: lastEcritureNum + 1, EcritureDate: line['date'], CompteNum: numeroCompte, PieceRef: line['facture correspondante'], EcritureLib: `vente : ${line['note'] || line['qui paye ?'] }`, Debit: '', Credit: convertToNumber(line['montant']) })
    ];
}
